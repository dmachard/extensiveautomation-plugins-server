...
from Var.SutAdapters import Codecs as LibraryCodecs
from Var.SutAdapters import Media as MediaTools
...
class DefaultPayloads(TestAdapterLib.Adapter):
	def __init__(self, parent, debug=False, codec=LibraryCodecs.A_G711U, rate=8000):
		"""
		"""
	...
	def getWhiteNoise(self, duration=500):
		...
		try:
			# generate samples
			noises = MediaTools.Noise(parent=self.testcase(), rate=self.rate)
	...
	def getSine(self, frequency=1000, duration=500):
		...
		try:
			# generate samples
			waves = MediaTools.Waves(parent=self.testcase(), rate=self.rate)
