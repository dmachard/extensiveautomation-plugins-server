...
class TESTCASE_ADAPTER_01(TestCase):
	...
	def prepare(self):
		...
		self.ADP_DUMMY = SutAdapters.Dummy.Adapter(parent=self, debug=False, shared=False)
...
class TESTCASE_AGENT_01(TestCase):
	...
	def prepare(self):
		# adapters and libraries
		self.AGT1_DUMMY = SutAdapters.Dummy.Agent(parent=self, agent=input('AGENT1'), debug=input('DEBUG'), shared=False)
		self.AGT2_DUMMY = SutAdapters.Dummy.Agent(parent=self, agent=input('AGENT2'), debug=input('DEBUG'), shared=False)
	def definition(self):
		# starting initial step
		self.step1.start()

		self.info( 'Send init command to agent: %s' % input('AGENT1') )
		self.AGT1_DUMMY.sendInitToAgent(data='init')
		self.AGT2_DUMMY.sendInitToAgent(data='init')
		self.wait(2)
		
		self.info( 'Send notify to agent: %s' % input('AGENT1') )
		self.AGT1_DUMMY.sendNotifyToAgent(data={'cmd':'dir'} )
		self.wait(2)
		
		self.info( 'Send reset command to agent: %s' % input('AGENT1') )
