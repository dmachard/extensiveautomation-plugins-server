...
class REGISTER_CALL_RAW_01(TestCase):	
	...
	def prepare(self, transport, displayRtpPackets, login, password):
		...
		self.sip = SutAdapters.SIP.Client( parent=self, debug=input('DEBUG'), destIp=input('DST_IP'), destPort=input('DST_PORT'), transport=transport, bindIp=input('SRC_IP'), sipPuid=input('PUID'), sipDomain=input('REALM'), sipDisplayName=input('DISPLAY_NAME') ) 
		
		# Initialize the sip adapter
		self.rtp = SutAdapters.RTP.Client( parent=self, debug=input('DEBUG'), bindIp=input('SRC_IP'), bindPort=input('SRC_PORT_RTP'), logLowLevelEvents=displayRtpPackets, recordRcvSound=input('RECORD_SOUND'), recordSndSound=input('RECORD_SOUND'), defaultSound=SutAdapters.RTP.SOUND_WHITE_NOISE	, payloadType=SutAdapters.Codecs.A_G711U ) 

		# Initialize the authenticator library
		self.rfc2617 = SutAdapters.Security.Digest(parent=self, debug=input('DEBUG'))
		
		# Initialize the SDP libraries
		self.sdp = SutAdapters.Media.SDP(parent=self, debug=input('DEBUG'), unicastAddress=input('SRC_IP'), connectionAddress=input('SRC_IP'))
...
class REGISTER_CALLER_01(TestCase):
	...
	def prepare(self, login, password):
		...
		self.sipPhone = SutAdapters.SIP.Phone( parent=self, rtpSrc=(input('SRC_IP'), 0), sipSrc=(input('SRC_IP'), 0), sipDest=(input('DST_IP'), input('DST_PORT')), debug=input('DEBUG'), enableTcp=False )
	...
	def definition(self, login, password):
		...
		if not self.sipPhone.isRegistered(timeout=input('TIMEOUT')):
			...
			sessid = self.sipPhone.placeCall( uri='bob-tas', recordCall=input('RECORD_SOUND'), 
																				playSound=SutAdapters.RTP.SOUND_SILENCE, codecSound=SutAdapters.Codecs.A_G711U,
...
class REGISTER_REG_RAW_SNIFFER_01(TestCase):	
	...
	def prepare(self, transport, login, password):
		...
		self.sip = SutAdapters.SIP.Client( parent=self, debug=input('DEBUG'), destIp=input('DST_IP'), destPort=input('DST_PORT'), 
																...
																sipDisplayName=input('DISPLAY_NAME'), sniffer=True ) 
...
class REGISTER_CALL_RAW_SNIFFER_01(TestCase):	
	...
	def prepare(self, transport, displayRtpPackets, login, password):
		...
		self.sip = SutAdapters.SIP.Client( parent=self, debug=input('DEBUG'), destIp=input('DST_IP'), destPort=input('DST_PORT'),
																	...
																	sipDisplayName=input('DISPLAY_NAME'), sniffer=True, bindEth=input('SRC_ETH') ) 
